...
func (rf *Raft) StartSnapshot(snapshot []byte, index int) {
	...
	var newLog []LogEntry
	newLog = append(newLog, LogEntry{Index:index, Term:rf.log[index-baseIndex].Term})
...
func (rf *Raft) RequestVote(args RequestVoteArgs, reply *RequestVoteReply) {
	...
	if args.Term < rf.currentTerm{
		...
		args.LastLogTerm == rf.log[len(rf.log)-1].Term && args.LastLogIndex < rf.GetLastLogIndex(){
...
func (rf *Raft) AppendEntries(args AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	}else if args.PrevLogIndex>baseIndex && rf.log[args.PrevLogIndex-baseIndex].Term != args.PrevLogTerm{
		reply.Success = false
...
func (rf *Raft) sendAppendEntriesHelper(server int){
	...
	appendEntriesArgs.Entries = rf.log[rf.nextIndex[server]-baseIndex:]
	...
	if rf.sendAppendEntries(server, appendEntriesArgs, appendEntriesReply){
		...
		if appendEntriesReply.Success{
			...
			//fmt.Printf("majority on %v, baseIndex=%v, log length=%v\n", N, baseIndex, len(rf.log))
...
func Make(peers []*labrpc.ClientEnd, me int,
	...
	log0 := LogEntry{Index:0, Term:0, Command:0}
	rf.log[0] = log0
	rf.commitIndex	= 0
	rf.lastApplied	= 0
